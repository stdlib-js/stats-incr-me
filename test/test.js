...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrme(), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'the accumulator function incrementally computes the mean error', function test( t ) {
	...
	for ( i = 0; i < N; i++ ) {
		...
		expected = sum / (i+1);
		actual = acc( x, y );
		t.strictEqual( isAlmostSameValue( actual, expected, 7 ), true, 'returns expected value' );
	}
	t.end();
});
...
tape( 'if not provided an input value, the accumulator function returns the current mean error', function test( t ) {
	...
	actual = acc();
	expected = 2.0 / 3.0;
	t.strictEqual( isAlmostSameValue( actual, expected, 3 ), true, 'returns expected value' );
	t.end();
});
